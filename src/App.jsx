...
function Av({name,color,size=44}){return <div style={{position:'relative',width:size,height:size,flexShrink:0,display:'inline-flex',alignItems:'center',justifyContent:'center',borderRadius:'50%',overflow:'hidden',boxShadow:'0 3px 10px rgba(0,0,0,0.28)'}}><img src='/golf_ball.webp' alt='' style={{width:'100%',height:'100%',objectFit:'cover',display:'block',borderRadius:'50%'}}/><span style={{position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-50%)',color:'#1a5c38',fontWeight:700,fontSize:size/2.5,lineHeight:1,pointerEvents:'none',userSelect:'none',zIndex:1,textShadow:'0 1px 2px rgba(255,255,255,0.6)'}}>{ini(name)}</span></div>}
...
function LoginScreen({onLogin}){
  ...
  return(
    <div style={{background:'#f5efe0',minHeight:'100vh',maxWidth:430,margin:'0 auto',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-start',paddingTop:'8vh'}}>
      <img src="/buddybet_iou_icon_256.png" alt="IOU Logo" style={{width:200,height:200,objectFit:'contain',marginBottom:8,filter:'drop-shadow(0 4px 12px rgba(0,0,0,0.15))}}/>
