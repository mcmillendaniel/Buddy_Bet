...
function Av({name,color,size=44}){return <div style={{position:'relative',width:size,height:size,flexShrink:0,display:'inline-flex',alignItems:'center',justifyContent:'center',borderRadius:'50%',boxShadow:'0 3px 10px rgba(0,0,0,0.28)'}}><svg viewBox='0 0 100 100' width={size} height={size} style={{display:'block'}}><defs><radialGradient id='bg' cx='38%' cy='32%' r='65%'><stop offset='0%' stopColor='#ffffff'/><stop offset='100%' stopColor='#d8dde8'/></radialGradient></defs><circle cx='50' cy='50' r='48' fill='url(#bg)'/>{[[50,22],[35,28],[65,28],[22,38],[50,38],[78,38],[30,50],[50,50],[70,50],[22,62],[50,62],[78,62],[35,72],[65,72],[50,78]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r='5.5' fill='none' stroke='rgba(180,190,210,0.7)' strokeWidth='0.8'/>)}</svg><span style={{position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-50%)',color:'#1a5c38',fontWeight:700,fontSize:size/2.5,lineHeight:1,pointerEvents:'none',userSelect:'none',zIndex:1}}>{ini(name)}</span></div>}
